...
def main():
    ...
    REGRESSION_CSV_PATH = './results/regression_log.csv'
...
def apply_log_transformation(train_data, test_data):
    train_transformed = np.sign(train_data) * np.log1p(np.abs(train_data))
    test_transformed = np.sign(test_data) * np.log1p(np.abs(test_data))
    return train_transformed, test_transformed, None


def inverse_log_transformation(transformed_data, scaler=None):
    return np.sign(transformed_data) * (np.exp(np.abs(transformed_data)) - 1)
...
def train_and_evaluate_regression(train_data: np.ndarray, test_data: np.ndarray,
    ...
    # Create regression datasets (these are in transformed space)
    ...
    # Inverse transform predictions and true targets back to original scale using the log inverse
    y_pred_test_orig = inverse_log_transformation(y_pred_test)
    y_test_orig = inverse_log_transformation(y_test)
    ...
    y_train_orig = inverse_log_transformation(y_train)
    y_train_pred_orig = inverse_log_transformation(y_train_pred)
...
def process_window_regression(window_config, data, forecast_horizon, hyperparams):
    ...
    # Apply log transformation to both training and testing data
    train_data_transformed, test_data_transformed, scaler = apply_log_transformation(train_data, test_data)

    return train_and_evaluate_regression(train_data_transformed, test_data_transformed, forecast_horizon,
