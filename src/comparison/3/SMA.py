...
def main():
    ...
    REGRESSION_CSV_PATH = 'results/moving_avg_model_10min.csv'
    FORECAST_HORIZONS_REG = [1]

    # Set the moving average window to 20 days worth of data for 10min intervals.
    # Assuming 144 samples per day: 20 days * 144 = 2880
    dummy_hp = {'look_back': 20 * 144}
...
# ==============================================================================
# Model: Forecast using Simple Moving Average
# ==============================================================================
def train_and_evaluate_regression(train_data: np.ndarray, test_data: np.ndarray,
    ...
    # Simple Moving Average forecast: use the average of the input sequence as the prediction.
    y_pred_test = np.mean(X_test, axis=1).reshape(-1, forecast_horizon)
    y_train_pred = np.mean(X_train, axis=1).reshape(-1, forecast_horizon)
