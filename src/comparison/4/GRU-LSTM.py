...
def calculate_profit_accuracy_and_transaction_frequency(y_true, probs, center_threshold=0.5, margin=0.1):
    ...
    # Total predictions is the total number of time steps (each is a potential trade)
    total_predictions = probs.size
    executed_trades = np.sum(trade_mask)

    # Calculate profit accuracy only for executed trades (if any)
...
# ==============================================================================
def bayesian_search_hyperparameters_classification(train_scaled, test_scaled, forecast_horizon, search_space,
                                                   time_limit=600):
    def objective(trial):
...
def save_results(results, csv_path):
    if results:
        ...
        cols_to_show = ['forecast_horizon', 'accuracy', 'precision', 'recall', 'f1_score', 'specificity',
...
def main():
    ...
    for horizon in FORECAST_HORIZONS_CLF:
        ...
        history = model.fit(X_combined, y_combined, epochs=best_hp['epochs'], batch_size=best_hp['batch_size'],
                            verbose=0)
        epochs_run = len(history.history['loss'])

        # Tune threshold on test set predictions using your original method
        ...
        # Now compute profit accuracy and transaction frequency using the new transaction strategy
