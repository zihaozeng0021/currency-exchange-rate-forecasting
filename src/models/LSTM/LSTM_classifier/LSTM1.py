...
def set_global_config(seed=42):
    """Set seed and suppress warnings for reproducibility."""
...
def load_data(data_path):
    """
    Load the CSV data file, remove infinities and NaNs,
    and return the 'Close' price series as a numpy array.
    """
...
def process_window_classification(window_config, data, forecast_horizon, hyperparams):
    ...
    apply independent scaling, and run classification training & evaluation.
    """
    n_samples = len(data)
    ...
    return optimize_and_train_classification(train_scaled, test_scaled, forecast_horizon, window_config['type'],
                                             hyperparams)
