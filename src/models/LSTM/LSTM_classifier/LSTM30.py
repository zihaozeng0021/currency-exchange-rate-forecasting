...
def optimize_and_train_classification(train_data, test_data, forecast_horizon, window_type, hyperparams, grid_threshold=None):
    ...
    # -----------------------------------------------------------
    # Cost-Sensitive Threshold Tuning on Validation Window
    # -----------------------------------------------------------
    ...
    if grid_threshold is None:
        ...
        for t in candidate_thresholds:
            ...
            # Compute cost using confusion matrix (assumes forecast_horizon == 1)
            cm = confusion_matrix(y_test.flatten(), preds_temp.flatten())
            if cm.shape == (2, 2):
                tn, fp, fn, tp = cm.ravel()
                cost = cost_FP * fp + cost_FN * fn
            else:
                # If one class is missing, assign a high cost if predictions do not match perfectly
                cost = 0 if np.all(y_test.flatten() == preds_temp.flatten()) else float('inf')
            if cost < best_cost:
                best_cost = cost
                best_threshold = t
