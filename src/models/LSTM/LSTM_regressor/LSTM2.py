...
def main():
    ...
    REGRESSION_CSV_PATH = './results/regression_results2.csv'
...
# ==============================================================================
def inverse_minmax_scaling(scaled_data, scaler):
...
def train_and_evaluate_regression(train_data: np.ndarray, test_data: np.ndarray,
    ...
    y_pred_test_orig = inverse_minmax_scaling(y_pred_test, scaler)
    y_test_orig = inverse_minmax_scaling(y_test, scaler)
    ...
    y_train_orig = inverse_minmax_scaling(y_train, scaler)
    y_train_pred_orig = inverse_minmax_scaling(y_train_pred, scaler)
...
def process_window_regression(window_config, data, forecast_horizon, hyperparams):
    ...
    train_data_scaled, test_data_scaled, scaler = apply_minmax_scaling(train_data, test_data)
