...
def main():
    ...
    CLASSIFICATION_CSV_PATH = 'results/LSTM_classification_hb.csv'
    ...
    for window in windows:
        ...
        for horizon in FORECAST_HORIZONS_CLF:
            print(f"\n--- Hyperband (Optuna) Search for forecast horizon {horizon} on validation split ---")
            best_hyperparams, best_threshold, best_val_result = hyperband_search_classification(
...
# ==============================================================================
# Hyperband Search for Hyperparameters using Optuna
# ==============================================================================
def hyperband_search_classification(window_config, data, forecast_horizon, search_space, time_limit=600):
    def objective(trial):
        hyperparams = {
        ...
        if result is None:
            return 0.0
        # Store the tuned threshold as a user attribute so we can retrieve it later.
        trial.set_user_attr("threshold", result['threshold'])
        return result['accuracy']

    # Create an Optuna study using HyperbandPruner.
    ...
    best_threshold = study.best_trial.user_attrs.get("threshold", None)
