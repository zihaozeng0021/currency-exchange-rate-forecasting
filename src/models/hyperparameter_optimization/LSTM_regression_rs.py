...
def main():
    ...
    REGRESSION_CSV_PATH = 'results/LSTM_regression_rs.csv'
...
# ==============================================================================
# Random Search for Hyperparameters using the Validation Set
# ==============================================================================
def random_search_hyperparameters(train_scaled, validate_scaled, scaler, forecast_horizon, search_space, time_limit=600):
    best_mse = float('inf')
    best_hp = None
    start_time = time.time()
    candidate_count = 0

    # Continue sampling candidates until the time limit is reached
    while time.time() - start_time <= time_limit:
        candidate_count += 1
        candidate = {}
        # Sample each hyperparameter from its interval
        for param, interval in search_space.items():
            min_val, max_val = interval
            if param in ['epochs', 'look_back', 'units', 'batch_size']:
                candidate[param] = random.randint(min_val, max_val)
            elif param == 'learning_rate':
                candidate[param] = random.uniform(min_val, max_val)
            else:
                candidate[param] = random.uniform(min_val, max_val)
        print(f"Evaluating candidate {candidate_count}: {candidate}")
        ...
        # Inverse transform predictions using the scaler fitted on the training data
    ...
    print(f"Best hyperparameters after evaluating {candidate_count} candidates: {best_hp} with validation MSE: {best_mse:.9f}")
    return best_hp
...
def process_window_regression(window_config, data, forecast_horizon, search_space):
    ...
    # Scale using training data only for random search
    scaler_train = StandardScaler()
    train_scaled = scaler_train.fit_transform(train_raw)
    validate_scaled = scaler_train.transform(validate_raw)

    # Perform random search on the validation set
    best_hp = random_search_hyperparameters(train_scaled, validate_scaled, scaler_train, forecast_horizon, search_space, time_limit=600)
