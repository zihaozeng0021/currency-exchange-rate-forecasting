...
def main():
    ...
    FTSE_DATA_PATH = './../../../../data/raw/dxy.csv'
    REGRESSION_CSV_PATH = './results/regression_dxy.csv'
    ...
    # Load and merge currency and FTSE data based on the Date column, then apply smoothing
...
# ==============================================================================
# Standard Scaling for train and test data
# ==============================================================================
...
# ==============================================================================
# Inverse Scaling for the Target Column
# ==============================================================================
def inverse_target_scaling(scaled_target, scaler, target_index=0):
    n_samples = scaled_target.shape[0]
    num_features = scaler.mean_.shape[0]
    dummy = np.zeros((n_samples, num_features))
    # Place the scaled target values in the appropriate column
    dummy[:, target_index] = scaled_target.flatten()
    inv_transformed = scaler.inverse_transform(dummy)
    return inv_transformed[:, target_index]
...
def create_dataset_regression(dataset, look_back=1, forecast_horizon=1):
    ...
    for i in range(len(dataset) - look_back - forecast_horizon + 1):
        ...
        # Regression target is the currency column (index 0)
...
def train_and_evaluate_regression(train_data: np.ndarray, test_data: np.ndarray,
    ...
    # X_train and X_test already have shape (samples, look_back, num_features)
    ...
    # Inverse transform predictions and true targets (for currency only)
    y_pred_test_orig = inverse_target_scaling(y_pred_test, scaler, target_index=0)
    y_test_orig = inverse_target_scaling(y_test, scaler, target_index=0)
    ...
    y_train_orig = inverse_target_scaling(y_train, scaler, target_index=0)
    y_train_pred_orig = inverse_target_scaling(y_train_pred, scaler, target_index=0)
...
def process_window_regression(window_config, data, forecast_horizon, hyperparams):
    ...
    # For multivariate data, train_raw and test_raw are already 2D (samples, num_features)
    # Apply Standard scaling to both training and testing data
    train_data_scaled, test_data_scaled, scaler = apply_standard_scaling(train_raw, test_raw)
